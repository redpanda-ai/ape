...
--	AND a.attribute_name = 'Item Name'
_tb_specs
...
WHERE
	v.vendor_id = __vendor_id
	 ...
	 AND i.enabled = 1
"
	disable_old_products = disable_template.gsub!(/__vendor_id/,$vendor_id)
	db = DBI.connect("dbi:ODBC:" + $dsn, $login, $passwd)
	dc = db.select_all(disable_old_products)
	db.disconnect
	#print disable_old_products
	#print "disable candidate length => " + dc.length.to_s()
